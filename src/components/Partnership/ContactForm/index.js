...
import {RECAPTCHA_SECRET, TANGEM_EMAIL_URI} from "../../../config";
...
const ContactForm = ({ program }) => {
    ...
    const recaptchaValue = refRecaptcha.current.getValue();
		...
		try {
			const response = await fetch(url, {
				...
				body: JSON.stringify({
					program, name, email, phone, message, recaptchaValue
				})
			});
  ...
  return (
    <form className={styles.form}>
      ...
      {isFailure && <span className={styles.failure}>{t('contactUs.result.failed')}</span>}
    </form>
  )
}
